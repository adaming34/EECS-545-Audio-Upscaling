...
class RingBuffer(Sequence):
	def __init__(self, capacity, cols=None, dtype=float, allow_overwrite=True):
		"""
		...
		cols: int, optional
			If provided, adds columns to the shape of the ring buffer
		allow_overwrite: bool
			If false, throw an IndexError when trying to append to an already
			full buffer
		"""
		self._arr = np.empty((capacity, cols) if cols else capacity, dtype)
		self._left_index = 0
		self._right_index = 0
	...
	def popall(self):
		right = self._right_index
		ret = np.concatenate((
			self._arr[self._left_index:min(right, self._capacity)],
			self._arr[:max(right- self._capacity, 0)]
		))
		
		self._left_index = right
